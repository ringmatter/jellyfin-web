define(['loading', 'events', 'libraryBrowser', 'imageLoader', 'listView', 'cardBuilder', 'globalize', 'emby-itemscontainer', 'scripts/elastic'], function (loading, events, libraryBrowser, imageLoader, listView, cardBuilder, globalize, elastic) {
    'use strict';


    function loadDownloads(tabContent, apiClient, user, params) {
    ...
    function reloadItems(page) {
        ...
        // const BASE_URL = 'http://nas.dubture.local/flask';
        ...
        return fetch(searchUrl, { method: 'POST', body: JSON.stringify(body), headers })
            ...
            .then(data => {
                ...
                var items = data.hits.hits.map((hit, idx) => {
                    ...
                    div += "<div class='listViewUserDataButtons'><button class='listItemButton paper-icon-button-light emby-button'>";
                    div += "<span data-id='" + md5 + "' class='material-icons delete playstatebutton-icon-unplayed'></span>"
                    div += "</button></div>"
                ...
                const updateDebug = (item) => {
                    ...
                    fetch(BASE_URL + '/preview', params).then(res => res.json()).then(data => {
                        if (!data.target) {
                ...
                document.querySelectorAll('span.delete').forEach(elem => {
                    elem.addEventListener('click', function(e) {
                        ...
                        loading.show();
                        fetch(BASE_URL + '/delete', params).then(res => res.json()).then(data => {
                            alert('deleted');
                            loading.hide();
                        });
                ...
                document.getElementById('downloadForm').addEventListener('submit', function (e) {
                    ...
                    loading.show();
                    fetch(BASE_URL + '/move', params).then(res => res.json()).then(data => {
                        alert('posted');
                        loading.hide();
                    });
                ...
                document.getElementById('create_target_path').addEventListener('click', function (e) {
                    ...
                    fetch(BASE_URL + '/create_target_path', params).then(res => res.json()).then(data => {
                        alert('Created');
                    });
